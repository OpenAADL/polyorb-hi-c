...
/*!
 * \brief Making Periodic Task consumer
 *
 ...
 * (pr_b_consumer_k) the second is the period defined in the AADL model. 
 * Third is the task priority ( 100), fourth is the stack size ( 0 bytes) 
 * and last is the subprogram executed by the task
 */
  __po_hi_create_periodic_task (pr_b_consumer_k, &(period), 100, 0, 0, consumer_job);
  
/*!
 * Store the period time for task result_producer
 */
  __po_hi_seconds (&(period), 5);
  
/*!
 * \brief Making Periodic Task result_producer
 *
 * Make a periodic task according to AADL model requirements. The first 
 * parameter is the task identifier defined in deployment.h 
 * (pr_b_result_producer_k) the second is the period defined in the AADL 
 * model. Third is the task priority ( 110), fourth is the stack size ( 0 
 * bytes) and last is the subprogram executed by the task
 */
  __po_hi_create_periodic_task (pr_b_result_producer_k, &(period), 110, 0, 0, result_producer_job);
