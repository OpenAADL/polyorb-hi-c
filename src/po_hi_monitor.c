...
int __po_hi_monitor_get_status_port(
  ...
  __PO_HI_DEBUG_DEBUG
    ("[MONITOR] call __po_hi_monitor_get_status_port with argument %d (port) and 0x%p (status pointer)\n",
     port, status);
  ...
  if (associated_device == invalid_device_id) {
    __PO_HI_DEBUG_CRITICAL
      ("[MONITOR] This port is not associated to a valid device (device-id=%d)\n",
       associated_device);
...
int __po_hi_monitor_get_status_device(
  ...
  __PO_HI_DEBUG_DEBUG
    ("[MONITOR] call __po_hi_monitor_get_status_device with argument %d (device) and 0x%p (status pointer)\n",
     device, status);
...
int __po_hi_monitor_get_status_bus(
  ...
  __PO_HI_DEBUG_DEBUG
    ("[MONITOR] call __po_hi_monitor_get_status_bus with argument %d (bus) and 0x%p (status pointer)\n",
     bus, status);
...
int __po_hi_monitor_report_failure_bus(
  ...
  __PO_HI_DEBUG_DEBUG
    ("[MONITOR] call __po_hi_monitor_report_failure_bus with argument %d (bus) and %d (failure)\n",
     bus, failure);
...
int __po_hi_monitor_report_failure_device(
  ...
  __PO_HI_DEBUG_DEBUG
    ("[MONITOR] call __po_hi_monitor_report_failure_device with argument %d (device) and %d (failure)\n",
     device, failure);
...
int __po_hi_monitor_report_failure_port(
  ...
  __PO_HI_DEBUG_DEBUG
    ("[MONITOR] call __po_hi_monitor_report_failure_port with argument %d (port) and %d (failure)\n",
     port, failure);
...
int __po_hi_monitor_recover_bus(
  const __po_hi_bus_id bus) {
  __PO_HI_DEBUG_DEBUG
    ("[MONITOR] call __po_hi_monitor_recover_bus with argument %d\n", bus);
  if ((bus < 0) || (bus >= __PO_HI_NB_BUSES)) {
    __PO_HI_DEBUG_CRITICAL("[MONITOR] invalid bus-id %d\n", bus);
    return __PO_HI_UNAVAILABLE;
  }
...
int __po_hi_monitor_recover_device(
  const __po_hi_device_id device) {
  __PO_HI_DEBUG_DEBUG
    ("[MONITOR] call __po_hi_monitor_recover_device with argument %d\n",
     device);
  if ((device < 0) || (device >= __PO_HI_NB_DEVICES)) {
    __PO_HI_DEBUG_CRITICAL("[MONITOR] invalid device-id %d\n", device);
    return __PO_HI_UNAVAILABLE;
...
int __po_hi_monitor_recover_port(
  ...
  __PO_HI_DEBUG_DEBUG
    ("[MONITOR] call __po_hi_monitor_recover_port with argument %d\n", port);
